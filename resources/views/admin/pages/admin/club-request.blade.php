            ...
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <div class="d-flex">
                    <div class="input-group">
                        <input type="text" class="form-control" id="search-activity"
                ...
                <tbody>
                    @foreach ($clubRequests as $request)
                        <tr>
                            ...
                            <td>
                                <span
                                    class="badge {{ $request->status == 'pending' ? 'bg-info bg-opacity-10 text-info' : ($request->status == 'approved' ? 'bg-success bg-opacity-10 text-success' : 'bg-danger bg-opacity-10 text-danger') }}">
                            ...
                            <div class="modal-dialog modal-lg">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="viewModalLabel-{{ $request->id }}">Chi tiết yêu cầu
                                            lập CLB</h5>
                                    ...
                                    <div class="modal-body">
                                        <div class="row">
                                            <div class="col-12 col-md-3">
                                                <img class=" rounded-circle" height="150" src="{{ $request->logo ? asset($request->logo) : asset('assets/client/images/logo-vnua.jpg') }}" alt="Avatar 1"> 
                                                <p><strong>Tên CLB:</strong> {{ $request->club_name }}</p>
                                            </div>
                                            <div class="col-12 col-md-9">
                                                
                                                <p><strong>Người yêu cầu:</strong> {{ $request->user->name }}</p>
                                                <p><strong>Email:</strong> {{ $request->user->email }}</p>
                                                <p><strong>Mô tả:</strong> {{ $request->description }}</p>
                                                <p><strong>Thời gian hoạt động:</strong> {{ $request->activity_time }}</p>
                        ...
                        <div class="modal fade" id="approveModal-{{ $request->id }}" tabindex="-1"
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="approveModalLabel-{{ $request->id }}">Duyệt yêu cầu
                                        </h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal"
                                            aria-label="Close"></button>
                                    </div>
                                    ...
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-secondary"
                                            data-bs-dismiss="modal">Đóng</button>
                                        <form action="{{ route('admin.club-requests.approve', $request->id) }}"
                                            method="POST">
                                            @csrf
                                            @method('PATCH')
                        ...
                        <div class="modal fade" id="rejectModal-{{ $request->id }}" tabindex="-1"
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="rejectModalLabel-{{ $request->id }}">Từ chối yêu cầu
                                        </h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal"
                                            aria-label="Close"></button>
                                    </div>
                                    ...
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-secondary"
                                            data-bs-dismiss="modal">Đóng</button>
                                        <form action="{{ route('admin.club-requests.reject', $request->id) }}"
                                            method="POST">
                                            @csrf
                                            @method('PATCH')
